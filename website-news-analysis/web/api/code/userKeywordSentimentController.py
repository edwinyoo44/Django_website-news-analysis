...
from api.code import userKeywordController as userkeyword_views
def load_df_data():
...
def get_daily_basis_sentiment_count(df_query, sentiment_type='pos', freq_type='D'):
    ...
    # x,y，用於畫趨勢線圖
    xy_line_data = [{'x':date.strftime('%Y-%m-%d %H:%M'),'y':freq} for date, freq in zip(freq_df_group.date_index,freq_df_group.frequency)]
    return xy_line_data
...
def filter_df_via_content(query_keywords, cond, cate, weeks):
    ...
    # start date
    start_date_delta = (datetime.strptime(end_date, '%Y-%m-%d %H:%M').date() - timedelta(weeks=weeks)).strftime('%Y-%m-%d %H:%M')
