...
def filter_dataFrame(user_keywords, cond, cate, weeks):
    ...
    # start date
    start_date = (datetime.strptime(end_date, '%Y-%m-%d %H:%M').date() - timedelta(weeks=weeks)).strftime('%Y-%m-%d %H:%M')
...
news_categories=['AI 人工智慧', '尖端科技', '市場動態', '生物科技', '網路', '能源科技', '行動裝置', '零組件', '全部']
...
def get_keyword_time_based_freq(df_query):
    ...
    for i, idx in enumerate(data.index):
        row = {'x': idx.strftime('%Y-%m-%d %H:%M'), 'y': int(data.iloc[i].freq)}
